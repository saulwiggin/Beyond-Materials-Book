...
Hx = zeros(ib,je);
Hy = zeros(ie,jb);
caEzx = ones(ib,jb); cbEzx = ones(ib,jb).*(dt/eps0/dx);
caEzy = ones(ib,jb); cbEzy = ones(ib,jb).*(dt/eps0/dx);
daHx = ones(ib,je); dbHx = ones(ib,je).*(dt/mu0/dx);
daHy = ones(ie,jb); dbHy = ones(ie,jb).*(dt/mu0/dx);
